    ...
    _base.init = function () {
      ...
      _base.append(
        '<a style="background-color:' +
          _settings._main_btn_color +
          '" href="javascript:void(0)" class="_col_shadow _collapz_parant _close contactFire"></a>',
      );
      ...
      if (_element.hasClass('_close')) {
        ...
        jQuery.each(child_attribute, function (i, val) {
          _child_el +=
            '<li><a href="' +
            val.url +
            '" class=""><div class="float-contact"><span class="_title ">' +
            val.label +
            '</span><div class="icon"><i class="' +
            val.icon +
            '"></i></div></div></a></li>';
        });
        _child_el += '</ul>';
